...
1. Read the canonical `skills/*/SKILL.md` files — never author new
   procedure content directly inside a platform wrapper.
...
5. Regenerate the common layer if stale: `project-profile.md`,
   `learned-rules.md`, `evals.md`, `failure-log.md`.
6. Diff Claude and Codex outputs side by side for the same source:
   wording may differ, behavior/rules must not.
7. Confirm no project-specific fact leaked into `GLOBAL_AGENTS.md` or
   canonical `skills/`.
...
- Leaking project-specific rules or facts into `GLOBAL_AGENTS.md` or
  canonical `skills/`.
