...
8. Write `.agent-os/project-profile.md`, `.agent-os/command-map.md`, `.agent-os/risk-map.md`, and an initial `.agent-os/architecture-map.md` (observed layers, dependency direction, boundaries, extension points), keeping observations and hypotheses in separate, labeled sections. The maps stay initial here — whole-repository deepening belongs to the `synthesize-project-maps` skill (builder-model pass).
