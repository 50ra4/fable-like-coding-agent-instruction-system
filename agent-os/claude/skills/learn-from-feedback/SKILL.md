...
3. Check whether the same substance already exists in the log or in `learned-rules.md` (and, if `.agent-os/rules/` exists, in `.agent-os/rules/*.md` — where the split-out active rules live). If it conflicts with an existing active rule, surface the conflict — do not silently overwrite either side. If you suspect a match is the same substance in different wording, consult `distill-rules`' clustering output (effective recurrence counts) instead of relying on literal string matching.
