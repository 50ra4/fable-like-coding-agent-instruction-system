...
5. If `.agent-os/context-checkpoints.md` has bloated, consolidate it back into one cumulative handoff summary: merge duplicate content, and mark obsolete direction as deprecated/superseded with a one-line reason instead of silently deleting it. Checkpoint content is working state, not rules — never treat it as active rules, and never promote it into `learned-rules.md` or the Global Layer directly; rule promotion goes through `learn-from-feedback`. This consolidation — evidence-backed demotion of unverified facts, re-compression into one cumulative summary, and contamination checks — can be delegated to the `audit-checkpoint` skill, which follows the same diff-and-approval protocol.
6. If `AGENTS.md`/`CLAUDE.md` contain long, step-by-step procedures, extract them into a new or existing skill under `skills/`, leaving a short pointer in the always-loaded file. Whether a statement belongs in the Global Layer or the project adapter at all — the cross-file layer-placement audit over `GLOBAL_*`, canonical skills, and adapter files — can be delegated to the `audit-layer-separation` skill, which consumes `validate-agent-os.sh`'s mechanical contamination warnings as its pre-pass and follows the same diff-and-approval protocol (demotion proposals and flags only; it never promotes content into the Global Layer).
...
8. For areas with a cluster of failures, propose new entries for `.agent-os/evals.md` using the standard Eval format so future regressions are caught automatically. Drafting these evals — clustering the failures by root cause and designing a Task and Forbidden behavior that only a model carrying the regression would fail — can be delegated to the `synthesize-evals` skill, which consumes `scripts/summarize-learning-log.sh` as its mechanical pre-pass.
